...
send_func_t send_func = send_hvc;
...
output_init_func_t output_init_func = output_init_hvc;
...
static int __init hvc_muen_init_console(int index, uint64_t epoch)
{
	 ...
	 * Otherwise we will loose log messages.
	 */
	if (early_out_len == 0)
 ...
 */
int __init early_get_str(const char *cmdline, const char *key, char *out, size_t out_size)
{
...
int __init early_get_u64(const char *cmdline, const char *key, u64 *out)
{
	char buf[32];
	int ret;
...
static int __init hvc_muen_earlycon_setup(struct earlycon_device *device, const char *opt)
{
	...
	if (strstr(boot_command_line, "hvc_muen.dbglog")) {
		send_func = send_dbglog;
		output_init_func = output_init_dbglog;
	}
